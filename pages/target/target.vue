	...
	<view class="bg">
		<view class="content">
			<view class="content-item">
				<lunc-calendar :showShrink="true" :showLunar="true" shrinkState="week" @dayChange="dayChange" @monthChange="monthChange" @shrinkClick="shrinkClick"></lunc-calendar>
            </view>
		</view>
		<view class="week-list">
			<view class="week-item" v-for="(item, index) in this.week_list" :key="item._id">
				<view class="week-left">
					<view class="week-name">
						{{item.week_name}} ({{ item.week_current }}/{{ item.week_reqest }})
						<view class="week-icon" v-if="item.week_award === 'flower'">
					...
					<view class="week-box">
						<view class="week-progress">
							<progress :percent="item.week_percent" border-radius="5" active stroke-width="16" />
						</view>
					</view>
				</view>
				<view class="week-button">
					<button :disabled="!item.week_is_done" style="width: 60px;height: 40px;font-size: 16px;"
